...
export function KombuchaGuide() {
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(
    new Set([kombuchaGuide.sections[0]?.id]) // First section expanded by default
  );
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = kombuchaGuide.sections.map(s => s.id);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <Card className="bg-card border-green-500/20 mb-8">
        ...
        {showTOC && (
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {kombuchaGuide.sections.map((section, index) => (
                <Button
                  key={section.id}
                  ...
                  className={`justify-start text-left h-auto py-2 px-3 ${
                    activeSection === section.id
                      ...
                      : 'text-cream hover:text-green-500 hover:bg-green-500/10'
                  }`}
                >
      ...
      {/* Introduction */}
      <Card className="bg-gradient-to-br from-green-500/10 to-teal-500/10 border-green-500 mb-8">
        <CardContent className="py-6">
          <p className="text-cream/90 leading-relaxed">
            Welcome to the world of kombucha brewing! This comprehensive guide will take you from absolute
            beginner to confident kombucha maker, covering everything you need to know to brew delicious,
            probiotic-rich kombucha at home. Safety, proper SCOBY care, and creative flavoring are all covered.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="space-y-8">
        {kombuchaGuide.sections.map((section, index) => (
          <Card
            ...
            className={`bg-card border-green-500/20 transition-all duration-300 ${
              activeSection === section.id ? 'ring-2 ring-green-500 shadow-lg shadow-green-500/20' : ''
            }`}
          >
            <CardHeader className="cursor-pointer" onClick={() => toggleSection(section.id)}>
              <CardTitle className="text-2xl text-green-500 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-green-600/60 text-lg">{index + 1}.</span>
                  {section.title}
                </div>
                <Button
                  variant="ghost"
                  ...
                  if (paragraph.startsWith('**') && paragraph.includes('**:')) {
                    ...
                    return (
                      <div key={idx} className="mb-4">
                        <h4 className="text-lg font-semibold text-green-500 mb-2">{headingText}</h4>
      ...
      {showBackToTop && (
        <Button
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 rounded-full w-12 h-12 shadow-lg bg-green-500 hover:bg-green-600 z-50"
          size="icon"
        >
