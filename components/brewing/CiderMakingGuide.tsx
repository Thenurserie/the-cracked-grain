...
export function CiderMakingGuide() {
  ...
  const [expandedSections, setExpandedSections] = useState<Set<string>>(
    new Set([ciderMakingGuide.sections[0]?.id]) // First section expanded by default
  );
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = ciderMakingGuide.sections.map(s => s.id);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <Card className="bg-card border-orange-500/20 mb-8">
        <CardHeader className="cursor-pointer" onClick={() => setShowTOC(!showTOC)}>
          <CardTitle className="text-orange-500 flex items-center justify-between">
            ...
            <Button
              variant="ghost"
              size="sm"
              className="text-orange-500 hover:text-orange-500/80 hover:bg-orange-500/10"
            >
        ...
        {showTOC && (
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {ciderMakingGuide.sections.map((section, index) => (
                <Button
                  key={section.id}
                  ...
                  className={`justify-start text-left h-auto py-2 px-3 ${
                    activeSection === section.id
                      ...
                      : 'text-cream hover:text-orange-500 hover:bg-orange-500/10'
                  }`}
                >
      ...
      {/* Introduction */}
      <Card className="bg-gradient-to-br from-orange-500/10 to-amber-500/10 border-orange-500 mb-8">
        <CardContent className="py-6">
          <p className="text-cream/90 leading-relaxed">
            Welcome to the world of cider making - the easiest and most forgiving fermented beverage you can make at home!
            This comprehensive guide will take you from absolute beginner to confident cider maker, covering everything from
            choosing the right juice to carbonation techniques. Whether you want dry, sweet, still, or sparkling cider, this guide has you covered.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="space-y-8">
        {ciderMakingGuide.sections.map((section, index) => (
          <Card
            ...
            className={`bg-card border-orange-500/20 transition-all duration-300 ${
              activeSection === section.id ? 'ring-2 ring-orange-500 shadow-lg shadow-orange-500/20' : ''
            }`}
          >
            <CardHeader className="cursor-pointer" onClick={() => toggleSection(section.id)}>
              <CardTitle className="text-2xl text-orange-500 flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <span className="text-orange-600/60 text-lg">{index + 1}.</span>
                  {section.title}
                </div>
                <Button
                  variant="ghost"
      ...
      {showBackToTop && (
        <Button
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 rounded-full w-12 h-12 shadow-lg bg-orange-500 hover:bg-orange-600 z-50"
          size="icon"
        >
