...
export function MeadMakingGuide() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const sections = meadMakingGuide.sections.map(s => s.id);
  ...
  return (
    <div className="max-w-5xl mx-auto">
      ...
      <Card className="bg-card border-yellow-500/20 mb-8">
        ...
        {showTOC && (
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
              {meadMakingGuide.sections.map((section, index) => (
                <Button
                  key={section.id}
                  ...
                  className={`justify-start text-left h-auto py-2 px-3 ${
                    activeSection === section.id
                      ...
                      : 'text-cream hover:text-yellow-500 hover:bg-yellow-500/10'
                  }`}
                >
      ...
      {/* Introduction */}
      <Card className="bg-gradient-to-br from-yellow-500/10 to-amber-500/10 border-yellow-500 mb-8">
        <CardContent className="py-6">
          <p className="text-cream/90 leading-relaxed">
            Welcome to the ancient art of mead making! This comprehensive guide will take you from absolute
            beginner to confident mead maker, covering everything you need to know to craft exceptional honey wine at home.
            Patience and proper nutrients are key—mead rewards those willing to wait.
          </p>
        </CardContent>
      </Card>
      ...
      <div className="space-y-8">
        {meadMakingGuide.sections.map((section, index) => (
          <Card
            ...
            className={`bg-card border-yellow-500/20 transition-all duration-300 ${
                  ...
                  if (paragraph.startsWith('**') && paragraph.includes('**:')) {
                    ...
                    return (
                      <div key={idx} className="mb-4">
                        <h4 className="text-lg font-semibold text-yellow-500 mb-2">{headingText}</h4>
      ...
      {showBackToTop && (
        <Button
          onClick={scrollToTop}
          className="fixed bottom-8 right-8 rounded-full w-12 h-12 shadow-lg bg-yellow-500 hover:bg-yellow-600 z-50"
          size="icon"
        >
